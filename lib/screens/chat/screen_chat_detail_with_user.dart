...
class ChatDetailWithUserScreen extends StatefulWidget {
  ...
  @override
  State<ChatDetailWithUserScreen> createState() =>
      _ChatDetailWithUserScreenState();
}
...
class _ChatDetailWithUserScreenState extends State<ChatDetailWithUserScreen> {
  ...
  Future<void> getChatRoomInfor() async {
    ...
    var dataResponse = await chatApi.shopJoinRoomChat(widget.idUser);
    if (dataResponse['isSuccess'] == true) {
      RoomInfor roomInfor = dataResponse['roomInfor'];
      idRoom = roomInfor.idRoom;
      idShop = roomInfor.idShop;
      var data = await joinWebSocket(idRoom, idShop);
  ...
  Future<Map<String, dynamic>> joinWebSocket(
      String idRoom, String idShop) async {
    var wsUrl = Uri.parse('${webSocketUrl}ws/joinRoom/$idRoom?id_shop=$idShop');
